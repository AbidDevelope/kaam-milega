      ...
      <table id="datatable" class="table table-bordered table-striped">
        <thead>
          <tr>
            ...
            <td>Desgination</td>
            <td>Company</td>
            <td>Presentation Pdf Link</td>
            <td>Video Link</td>
            <td>Action</td>
          </tr>
        </thead>
        @foreach ($professor as $key=>$data)
        <tr>
          <td>{{ ++$key }}</td>
          <td>{{$data->module_name}}</td>
          <td>
            @if(!empty($data->professor_info))
            @foreach($data->professor_info as $key => $img)
            <p>
            {{$img['name']}}
            </p>
            @endforeach
          ...
          <td><a target="_blank" href="{{ asset('public/uploads/'.$data->presentaion_pdf) }}" class="imgThumb">Link</a></td>
          <td><a target="_blank" href="{{ $data->video_link }}" class="imgThumb">Link</a></td>
