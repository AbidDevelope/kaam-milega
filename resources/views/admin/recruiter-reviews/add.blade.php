    ...
    <form role="form" method="post" action="{{ url($base_url) }}" enctype="multipart/form-data">
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control @error('company_name') is-invalid @enderror" value="{{ old('company_name') }}"
            id="company_name" name="company_name" placeholder="Enter company_name">
          @error('company_name')
          <div class="text-danger" role="alert">{{ $message }}</div>
          @enderror
        </div>
