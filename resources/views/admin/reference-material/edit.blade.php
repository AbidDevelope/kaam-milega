  ...
  <div class="card card-primary">
    ...
    <form role="form" method="post" action="{{ url($base_url.'/'.$id) }}"  enctype="multipart/form-data">
      ...
      <div class="card-body">
        ...
        <div>
          <a href="{{ asset('public/uploads/'.$pdf) }}" class="imgThumb" target="_blank">Link</a>
        </div>
        <div class="form-group">
          <label for="name">Pdf</label>
          <input type="file" class="form-control @error('pdf') is-invalid @enderror" accept="application/pdf" value="{{ $pdf }}" id="pdf" name="pdf" placeholder="Enter pdf">
          @error('pdf')
          <div class="text-danger"  role="alert">{{ $message }}</div>
          @enderror
        </div>
